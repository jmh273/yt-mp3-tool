# concurrent-downloads Specification

## Purpose
定義批次下載的並行執行機制：以設定（`download_concurrency`）控制的並發上限，透過背景執行緒 + 號誌（semaphore）同時進行多支影片的「下載 + 轉檔」pipeline，縮短整批耗時。涵蓋並發上限的讀取與夾限、並行下序號前綴依批次索引計算（與完成順序解耦）的正確性，以及「全部影片結束才判定任務完成」的語意。對前端透明：不改動 `POST /download` payload 與 SSE 進度結構。
