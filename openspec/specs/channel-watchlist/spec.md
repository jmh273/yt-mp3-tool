...
定義「觀察名單」capability：一份儲存在前端 localStorage 的頻道清單，依登入帳號隔離，不存於後端、不跨裝置同步。觀察名單作為「同類新頻道」探索與「訂閱頻道」之間的中間狀態，讓使用者把感興趣但尚未決定訂閱的頻道暫存起來；後續可在觀察名單面板瀏覽該頻道近期影片，並一鍵升級為正式訂閱（呼叫 `POST /subscriptions/{channel_id}`，扣 50 quota）。觀察名單只記錄頻道層級資料（`channel_id`、`title`、`thumbnail`、`added_at`），不快取影片清單。
