...
## Purpose
定義「待下載選取」與「下載失敗狀態」在頁面重整/重啟後仍可續的最小機制（Level 1）：選取清單持久化於瀏覽器 localStorage、下載結束後失敗項仍可見，並沿用既有「失敗項留在選取清單」的路徑一鍵重試。對後端透明：不改 `POST /download` payload、不改 SSE 結構、不新增後端端點。
