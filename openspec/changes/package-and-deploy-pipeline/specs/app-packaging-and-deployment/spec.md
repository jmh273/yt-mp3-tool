...
The repository SHALL include `scripts/update.bat` (also shipped inside the release zip). When run on a target Windows PC with the environment variable `GH_TOKEN` set to a fine-grained PAT with `contents: read` scope, it SHALL: (1) query the GitHub API for the latest release; (2) compare its tag with the local `_version.txt`; (3) if newer, download the windows-x64 zip, stop any running `yt-mp3-tool.exe`, extract over the install directory, and start the new exe. The script SHALL leave `~/.yt-mp3-tool/` and the user's MP3 download directories untouched.
...
- **WHEN** the user runs `update.bat` without `GH_TOKEN` set
- **THEN** the script SHALL print an error pointing to DEPLOY.md (how to create a PAT) and exit non-zero

#### Scenario: Network unreachable
- **WHEN** the GitHub API call fails (no network / DNS failure)
