...
> 💡 若按「建立」後出現要求**啟用兩步驟驗證（2-Step Verification）**的提示：
> 依指示到 Google 帳號設定開啟兩步驟驗證，完成後**重新整理（refresh）這個頁面**，才能繼續建立專案。
...
1. 「API 和服務」→「OAuth 同意畫面」。新版會進到 **「Google 驗證平台 / OAuth 總覽」** 頁。
   - 若看到「**尚未設定 Google 驗證平台**…」與下方的 **「開始」** 按鈕 → 按「開始」進入設定精靈。
   - （舊版介面則會直接要你選 User Type，對應下面的「目標對象」步驟。）
2. 精靈會分幾步要你填，依序：
   - **應用程式資訊**：應用程式名稱（隨意，例如 `yt-mp3-tool`）、使用者支援電子郵件（選你自己）。
   - **目標對象（Audience）**：選 **外部（External）**。⭐（這就是舊版的 User Type）
   - **聯絡資訊**：填你自己的 email。
   - 勾選同意政策 → **建立**。
3. 建立完成後，左側會出現「目標對象（Audience）」「用戶端」「資料存取」等分頁。
4. 「資料存取 / Scopes」**不用**自己加，本工具會在登入時動態請求所需 scope。
5. **測試使用者**：到「**目標對象（Audience）**」分頁 →「測試使用者」→「新增使用者」→ 把**你自己的 Google 帳號 email**加進去 → 儲存。

#### ⭐ 重要：把同意畫面「發布到正式」

設定完成後，到「**目標對象（Audience）**」分頁（舊版是「OAuth 同意畫面」頁），會看到發布狀態是**「測試」**。
...
> 先確認**頂端的專案仍是 `yt-mp3-tool`**。新版 Console 在「Google 驗證平台」各分頁間切換時，有時會跳離專案脈絡；若發現不是，回頂端專案下拉重新選一次。

1. 從左上角 **☰**（或「**Google Cloud**」標誌）叫出選單 →「**API 和服務**」→「**憑證**」→「**建立憑證**」→「**OAuth 用戶端 ID**」。
   （也可從「Google 驗證平台」的「**用戶端（Clients）**」分頁 →「**建立用戶端**」，兩條路結果相同。）
2. 應用程式類型選 **電腦版應用程式（Desktop app）**。⭐（一定要選這個；別選網頁應用程式等其他類型，否則 localhost 授權會失敗）
3. 取個名字 → 建立。
4. 建立後的對話框點 **「下載 JSON」**。
